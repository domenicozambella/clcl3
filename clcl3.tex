...
\begin{fact}\label{fact_otto}
  ...
  Then the following are equivalent for every tuple $C$\smallskip
    ...
    \ceq{\hfill M}{\models}{\phantom{\neg}\varphi(C)\ \rightarrow\ \bigwedge\Big\{\ \varphi(C')\ :\ C'\textrm{ neighborhood of }C\Big\} }\smallskip
...
Let $\varphi(x\,;z\,;X)$ be an infinite conjunction of formulas in ${\EuScript F}_{\sf X}$. The variables of sort ${\sf H}$ are partitioned in two tuples $x\,;z$ which may be infinite.
...
\ceq{\hfill d\big(f(b)\;,\tau(a\,;b)\big)}{\le}{\varepsilon}\hfill for every $b\in B$.
...
We write $K(S)$ for a set of compact subsets of $S$ or, when the context suggest it, of $S^n$.  
...
Note that ${\EuScript D}\subseteq {\sim}{\sim}{\EuScript D}$.
We say that ${\EuScript D}$ is regular if ${\EuScript D}= {\sim}{\sim}{\EuScript D}$.
...
\begin{fact}
  For every ${\EuScript D}\subseteq {\EuScript U}^z\times K(S)$ the following are equivalent
  \begin{itemize}
    \item [1.] ${\EuScript D}$ is approximable by $\varphi(x\,;z\,;X)$
    \item [2.] the following defines a (consistent) global $\varphi(x\,;z\,;X)$-type
    
    \noindent\kern-\labelwidth\kern-\labelsep
    \ceq{\hfill p(x)}{=}{\big\{\varphi(x\,;b\,;C)\ :\ \langle b,C\rangle\in{\EuScript D}\big\}\ \cup\ \big\{{\sim}\varphi(x\,;b\,;C)\ :\ \langle b,C\rangle\in{\sim}{\EuScript D}\big\}.}
