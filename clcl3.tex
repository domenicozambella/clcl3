...
\footnotesize
  ...
  \def\address##1##2{\begingroup
    ...
    \@ifnotempty{##1}{\ignorespaces##1\unskip\quad  }%
...
\begin{abstract}
  Recently, a classical approach to continuous structures has been proposed in \cite{clcl} and~\cite{Z} that extends the class of structures falling under the scope of~\cite{BBHU}.
  These articles introduce the notion of \textit{structures with a standard sort.}
  We discuss local stability in this context.
  We examine three variants of the order property which are prima facie non equivalent.
  For each variant we show that sets externally definable by stable formulas are definable in some appropriate sense.
\end{abstract}
...
Structures with a standard sort are introduced in \cite{clcl} as a framework to the model theory continuous structures.
This framework extends the range of structures that can be described by continuous logic \`a la~\cite{BBHU}.
The underlying logic is classical, but compactness only holds for positive formulas.
We refer the reader to the introduction of \cite{clcl} or~\cite{Z} for motivation and further background.
In this section we recall the definitions and basic properties are needed to make this paper self-contained.

There is more than one way to define stability in the context introduced in \cite{clcl}.
It is also not clear how to generalize the notion of definability of types (in Fact~\ref{fact_approx_X} we introduce a definition which seems robust).
We discuss these issues in Sections~\ref{finitary}, \ref{nonfinitary} and~\ref{epsilonstable}.
We consider three variants of the notion of stability and for each we prove a suitable version of the classical theorem that says that externally definable sets are definable.
...
Throughout the paper the letter $C$ always denotes a compact subset of $S$, or a tuple of such sets.

In ${\EuScript L}_{\sf S}$ we could also allow relation symbols for all compact subsets of $S^n$, for every $n$.
...
We also fix an arbitrary first-order language which we denote by \emph{${\EuScript L}_{\sf H}$,} and which  we call the language of the home sort.
...
The setting in \cite{CP} is a generalization of that used by Henson and Iovino for Banach spaces (see, for example,~\cite{I}).
...
In \cite{Z} it is observed that, under the assumption of equicontinuity, the proof in \cite{clcl} extends to the case $n{\cdot}m> 0$ (a case which captures examples such as equicontinuous flows).
...
In this section we introduce some notions that are used to describe global types in Section~\ref{nonfinitary}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Stable formulas~--~the finitary case}\label{finitary}
\def\medrel#1{\parbox{5ex}{\hfil $#1$}}
\def\ceq#1#2#3{\parbox[t]{22ex}{$\displaystyle #1$}\medrel{#2}{$\displaystyle #3$}}

In this section we consider the strongest possible notion of order property.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Stable formulas~--~the non-finitary case}\label{nonfinitary}
\def\medrel#1{\parbox{5ex}{\hfil $#1$}}
\def\ceq#1#2#3{\parbox[t]{22ex}{$\displaystyle #1$}\medrel{#2}{$\displaystyle #3$}}
...
\begin{fact}\label{fact_approx_X}
  Let $\varphi(x\,;z\,;X)$ be an ${\EuScript F}_{\sf X}$-formula.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Yet another version of stability}\label{epsilonstable}
\def\medrel#1{\parbox{5ex}{\hfil $#1$}}
...
\vskip10ex
\textbf{Acknowledgments}\quad Domenico Zambella would like to thank Elisabetta Marchese for helpful discussion.


% \textbf{Author contribution}\quad All authors have contributed equally.

\textbf{Funding}\quad
Silvia Barbina was partially supported by PRIN2022 \textit{Models, sets and classifications}, prot.\@ 2022TECZJA.
Riccardo Camerlo was partially supported by the MUR excellence department project awarded to the Department of Mathematics of the University of Genoa, CUP D33C23001110001. 
Domenico Zambella was partially supported by PRIN2022 \textit{Logical methods in combinatorics}, prot.\@ 2022BXH4R5

% \textbf{Conflict of interest}\quad The authors declare no conflict of interest.
...
% }\smallskip
% \vskip-\baselineskip
% \textcolor{blue}{\bib{CH}{article}{
%   label={CH},
%   title={Piecewise Interpretable Hilbert Spaces}, 
%   author={Alexis Chevalier},
%   author={Ehud Hrushovski},
%   note={\href{https://arxiv.org/abs/2110.05142}{arXiv:2110.05142}},
%   date = {2023},
% }}\smallskip
\bib{CP}{article}{
    label={CP},
