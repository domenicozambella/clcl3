...
\begin{fact}\label{fact_stability_semicalssic}
  The following are equivalent
  \begin{itemize}
    \item [2.]  $\varphi(x\,;z\,;C)$ is finitarily stable
    \item [3.]  for some $m$ there is no sequence $\langle a_i\,;b_i\ :\ i<m\rangle$ such that for some $\tilde C\cap C=\varnothing$\smallskip
...
\begin{proof}
  ...
  Then the union of the $\tilde C_{n,i}$ is the set $\tilde C$ required to negate (3).
\end{proof}
...
\begin{remark}\label{rem_stable}
  ...
  \ceq{\hfill \sigma(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;\tilde C)}\hfill for every $i<n<\omega$ or for every $n<i<\omega$.\smallskip
\end{remark}
...
Note also that, by the pigeonhole principle, we can strengthen the requirement in Definition~\ref{def_stable} by requiring that there is no $\langle a_i\,;b_i\,;C_i\,;\tilde C_i\ :\ i<\kappa\rangle$ such that $\tilde C_i\cap C_i=\varnothing$ and\smallskip
...
\begin{remark}\label{rem_approx}
  If ${\EuScript D}$ is involutive the following are equivalent for every small set $B\subseteq{\EuScript U}^z$\smallskip
...
Under the assumption of the remark (in particular, for formulas in place of types) it is easy to verify by compactness that in Definition~\ref{def_approx} the requirement \textit{for every small\/} $B$ can be replaced by \textit{for every finite\/} $B$.
