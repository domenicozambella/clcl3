...
We introduce a new sort ${\sf X}$ with the purpose of conveniently describing classes of ${\EuScript F}$-for\-mulas that only differ by the sets/predicates $C\subseteq S$ they contain.
...
We say that $M$ is \emph{${\EuScript F}$-maximal\/} if it models $\varphi\in{\EuScript F}(M)$ whenever $\varphi$ holds in some ${\EuScript F}$-elementary extensions of $M$.
...
\begin{notation}
  Let ${\EuScript U}$ be an ${\EuScript F}$-saturated standard structure.
...
Note that we are requiring closure in two distintic contexts: topological, and Boolean (upward closere by inclusion).
...
\begin{proof}
  ...
  The implication $\Rightarrow$ in the definition of closed is obvious.
...
We say that $\varphi(x\,;z\,;C)$ is  \emph{finitarily stable\/} if for some $m<\omega$ there is no sequence $\langle a_i\,;b_i\ :\ i<m\rangle$ such that for every $i<n<m$\smallskip
...
\begin{fact}\label{fact_stability_semicalssic}
  ...
  \begin{itemize}
    ...
    \item [2.]  for some $m$ there is no sequence $\langle a_i\,;b_i\ :\ i<m\rangle$ such that for some $\tilde C$ disjoint of $C$ and for every $i<n<m$\smallskip
...
We say that ${\EuScript D}$ is involutive/closed if all its fibers are.
...
By this remark it is easy to see that, by compactness, when $\sigma$ is a formula it is sufficient to verify the above definition for $B$ finite.
