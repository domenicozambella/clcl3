...
Let $\varepsilon$ a neighborhood of the diagonal of $S^2$.
For $a,b\in{\EuScript U}^n$ we write $a\sim_\varepsilon b$ if $\langle\tau(a),\tau(b)\rangle\in\varepsilon$ for every term $\tau(z)$ of sort ${\sf H}^n\to{\sf S}$.
...
\begin{theorem}
  ...
  \begin{itemize}
    ...
    \item [2.] $C\in{\EuScript D}\medrel{\Leftrightarrow}C'\in{\EuScript D}$\hfill for every neighborhood $C'$ of $C$.
  \end{itemize}
\end{theorem}
...
\begin{proof}
  ...
  Implication $\Rightarrow$ is ovious.
...
\begin{theorem}[ (conjecture)]\label{fact_instable3}
...
We say that ${\EuScript D}$ is involutive if all its fibers are such.

We write $\varepsilon$ for a compact neighborhood of the diagonal of $(S^n)^2$.
We write $C^\varepsilon$ for elements of $S^n$ that are $\varepsilon$-close to $C$.

We say that $\varphi(x\,;z\,;C)$ is \emph{$\varepsilon$-stable\/} if the equivalent conditions in the following fact hold.

\begin{theorem}[ (conjecture)]\label{fact_instable3}
  The following are equivalent\smallskip

  \begin{itemize}
    \item [1.]  there is no sequence $\langle a_i\,;b_i\ :\ i<\omega\rangle$ such that for some $\tilde C\cap C^\varepsilon=\varnothing$ and every $i<n$

    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;\tilde C)}

   \item [2.] there is no sequence $\langle a_i\,;b_i\,;C_i\ :\ i<\omega\rangle$ such that for every $i<n$
  
    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;C_n)}\quad and\quad $C^\varepsilon\cap C_n=\varnothing$.

    \item [3.]  there is a maximal $m$ such that some sequence $\langle a_i\,;b_i\,;C_i\ :\ i<m\rangle$ is such that for every $i<n$
...
\begin{definition}\label{def_approx_X}\strut
  ...
  \ceq{2.\hfill\varphi(a\,;b\,;C^\varepsilon)}{\Rightarrow}{\langle b,C\rangle\in {\EuScript D}}\hfill for every $b\in B$ and every $C$.\smallskip

We say $\varepsilon$-approximable \emph{from below\/} if the implication $\Leftarrow$ holds for every $b\in{\EuScript U}^z$.
We say \emph{approximable\/} for $\varepsilon$-approximable for every $\varepsilon$.
\end{definition}

By Facts~\ref{fact_~definibile} and~\ref{fact_~inclusione} when ${\EuScript D}$ is involutive (1) can be rephrased as

\ceq{\hfill{\sim}\varphi(a\,;b\,;C)}{\Rightarrow}{\langle b,C\rangle\in{\sim}{\EuScript D}}\hfill for every $b\in B$ and every $C$.

Analogously, (2) can be rephrased as
...
\begin{proof}
  ...
  \ceq{\hfill \langle b,C\rangle\in{\sim}{\EuScript D}}{\Rightarrow}{{\sim}\varphi(a\,;b\,;C^\varepsilon).}
    
  By what remarked above, these yield (1) and (2) in Definition~\ref{def_approx_X}.
  
  1$\Rightarrow$2.
  Firstly, we prove finite consistency.
  Let $B$ and $C_1,\dots,C_n$ be given.
  Let $\varepsilon$ be small enough so that 
  
  \ceq{\hfill\langle b,C_i^\varepsilon\rangle\in{\sim}{\EuScript D}}{\Rightarrow}{\langle b,C_i\rangle\in{\sim}{\EuScript D}}\hfill   for every $b\in B$ and every $i<n$.
  
  
  Let $a$ be given by Definition~\ref{def_approx_X}.
  Then 

  \ceq{\hfill\langle b,C_i\rangle\notin{\EuScript D}}{\Rightarrow}{C_i\cap C=\varnothing} for some C\

  \ceq{\hfill\langle b,C_i\rangle\in{\EuScript D}}{\Rightarrow}{\varphi(a\,;b\,;C_i)}\hfill for every $b\in B$ and every $i$.

  Then the following holds for every $\varepsilon$

  \ceq{\hfill \langle b,C\rangle\in{\sim}{\EuScript D}}{\Rightarrow}{{\sim}\varphi(a\,;b\,;C_i^\varepsilon)}
  
  As $\varepsilon$

  \ceq{}{\Rightarrow}{\varphi(a\,;b\,;C_i).}
  



  This implies that $\langle b,C_i\rangle\in{\EuScript D}$ for every $b\in B$ and every $C_i$.

  
  

  Then for every $i<n$ and every $b\in B$ we have that
  
  follows (again, immediately) from approximability.
