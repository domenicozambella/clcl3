...
We associate to $S$ a first order structure in a language \emph{${\EuScript L}_{\sf S}$\/} that has a symbol for each regular compact subset $C\subseteq S$ and a function symbol for each continuous functions $f:S^n\to S$.
...
Throughout these notes the letter $C$ always denotes a regular compact subset of $S$, or a tuple of such sets.
...
\begin{definition}
  ...
  \begin{itemize}
  \item[i.] atomic and negated atomic formulas of ${\EuScript L}_{\sf H}$ with exlusion of equalities and inequalities
  \item[ii.] $\tau\in C$, where $\tau$ is a term of sort ${\sf H}^n\times {\sf S}^m\to {\sf S}$, and $C\subseteq S$ is a regular compact set.
  \end{itemize}
\end{definition}
...
If $C=C _1,\dots,C_k$ is a tuple of regular compact subsets of $S$ then $\varphi(x\,;\xi;C)$, is a formula in ${\EuScript F}$.
...
In this section ${\EuScript D}$ and ${\EuScript C}$ range over the subsets of $\subseteq K(S)^n$, for some $n$.
We define 
...
\begin{fact}\label{fact_~definibile}
  For every $\varphi(X)\in{\EuScript F}_{\sf X}({\EuScript U})$, if ${\EuScript D}=\{C\,:\,\varphi(C)\}$ then ${\sim}{\EuScript D}=\{\tilde C\,:\,{\sim}\varphi(\tilde C)\}$
\end{fact}
...
\ceq{\hfill C\in{\EuScript D}}{\Leftrightarrow}{C'\in{\EuScript D}}\hfill for every neighborhood $C'$ of $C$.

Note that we are requiring closure in two distintic contexts: topological, and Boolean algebraic (upward clousere by inclusion).
...
Therefore we intruduce the non finitary version of saturation as a separate notion.
Namely, we say that $\varphi(x\,;z\,;C)$ is \emph{stable\/} if the equivalent conditions in the following fact hold.

\begin{theorem}[ (conjecture)]\label{thm_epsilon_stable}
  The following are equivalent\smallskip

  \begin{itemize}
    \item [1.]  there is no sequence $\langle a_i\,;b_i\ :\ i<\omega\rangle$ such that for some $\tilde C\cap C=\varnothing$ and every $i<n$

    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;\tilde C)}

   \item [2.] there is no sequence $\langle a_i\,;b_i\,;C_i\ :\ i<\omega\rangle$ such that for some neighboorhood $C'$ of $C$ and for every $i<n$
  
    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;C_n)}\quad and\quad $C'\cap C_n=\varnothing$.

    \item [3.]  for every neighboorhood $C'$ of $C$ there is a maximal $m$ such that some sequence $\langle a_i\,;b_i\,;C_i\ :\ i<m\rangle$ is such that for every $i<n$\smallskip

    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;C_n)}\quad and\quad $C'\cap C_n=\varnothing$.    
  \end{itemize}
\end{theorem}

The following two definition are classical, i.e.\@ they rely on the full language ${\EuScript L}$.
...
We state it only for comparison with its non finitary counterpart, see Theorem~\ref{thm_stable_definable}.
...
In this section  ${\EuScript D}$ is always a subset of ${\EuScript U}^z\times K(S)^n$, where $n$ has to be inferred from the context (typically, $n=|X|$).
...
If $C$ is a tuple, the definition above applies componentwise.

We say that $\varphi(x\,;z\,;X)$ is \emph{$\varepsilon$-stable\/} if the equivalent conditions in the following fact hold.

\begin{theorem}[ (conjecture)]\label{thm_epsilon_stable}
  The following are equivalent\smallskip

  \begin{itemize}
    \item [1.]  there is no sequence $\langle a_i\,;b_i\ :\ i<\omega\rangle$ such that for some $C$, some $\tilde C\cap C^\varepsilon=\varnothing$, and for every $i<n$

    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;\tilde C)}\smallskip

   \item [2.] there is no sequence $\langle a_i\,;b_i\,;C_i\ :\ i<\omega\rangle$ such that for some $C$ and for every $i<n$
  
    \ceq{\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;C_n)}\quad and\quad $C^\varepsilon\cap C_n=\varnothing$.\smallskip

    \item [3.]  there is a maximal $m$ such that some sequence $\langle a_i\,;b_i\,;C_i\ :\ i<m\rangle$ is such that for some $C$ and for every $i<n$
...
We say approximable \emph{from below\/} if the second implication holds for every $b\in{\EuScript U}^z$.
\end{definition}
...
\ceq{\hfill \langle b,C\rangle\in{\sim}{\EuScript D}}{\Rightarrow}{{\sim}\varphi(a\,;b\,;C^\varepsilon)}\hfill for every $b\in B$ and every $C$.
...
\begin{proof}
  ...
  \ceq{}{~}{\big\{{\sim}\varphi(x\,;b\,;C^\varepsilon)\ :\ \langle b,C\rangle\in{\sim}{\EuScript D},\text{ and all }\varepsilon\big\}.}
  
  Let $B$ and $\varepsilon$ be given.
...
\begin{lemma}\label{lem_epsilon_stability_definability}
...
\begin{proof}
  The lemma is an immediate consequence of the following three.
\end{proof}

\begin{lemma}
  Under the assumptions of Lemma~\ref{lem_epsilon_stability_definability}.
  If ${\EuScript D}$ is approximable from below then there is a sequence $\langle a_i\ :\ i<k\rangle$ such that for every $b\in{\EuScript U}^z$\medskip
...
\ceq{3.\hfill \langle b,C\rangle\in{\EuScript D}}{\Leftarrow}{\varphi(a_n\,;b\,;C)}\hfill for every $b\in{\EuScript U}^z$

and

\ceq{4.\hfill \langle b_i,C\rangle\in{\EuScript D}}{\Rightarrow}{\varphi(a_n\,;b_i\,;C)}\hfill for every $i<n$.\smallskip

This is possible because ${\EuScript D}$ is approximated from below.
Note that (3) immediately guarantees (2).
Now choose $b_n$ and $C_n\cap C^\varepsilon=\varnothing$ such that

\ceq{5.\hfill \langle b_n,C\rangle\in{\EuScript D}}{\&}{\bigwedge_{i=0}^n\ {\sim}\varphi(a_i\,;b_n\,;C_n)}%\smallskip

If such $b_n$ and $C_n$ do not exist, then 

\ceq{\hfill \langle b,C\rangle\in{\EuScript D}}{\Rightarrow}{\neg\,{\sim}\bigvee_{i=0}^n\ \varphi(a_i\,;b\,;\tilde C)}\ \hfill for every $b\in{\EuScript U}^z$ and every $\tilde C\cap C^\varepsilon=\varnothing$%\smallskip

which is equivalent to (1) by Fact~\ref{fact_otto} and proves the lemma.
Suppose for a contradiction that the construction never ends.
Then, from (4) we otain $\varphi(a_n\,;b_i\,;C)$ for every $i<n$ while from (5) we obtain ${\sim}\varphi(a_i\,;b_n\,;C_n)$, for every $i\le n$.
This contradicts stability.
\end{proof}
...
\begin{lemma}
  ...
  \ceq{\hfill\sigma(\bar x\,;z\,;X)}{=}{\bigwedge_{i\le m}\ \varphi(x_i\,;z\,;X)}\medskip
...
\begin{proof}
  ...
  \ceq{3.\hfill \langle b,C\rangle\notin{\EuScript D}}{\Rightarrow}{{\sim}\varphi(a_n\,;b\,;\tilde C)}\hfill for some $\tilde C\cap C^\varepsilon=\varnothing$.

  We write $C_i$ for the $\tilde C$ that witnesses (3) for $b_i$.
  Now, as the lemma is assumed to fail, we can choose $b_n$ such that

  \ceq{4.\hfill \langle b_n,C\rangle\notin{\EuScript D}}{\&}{\bigwedge_{i=0}^n\ \varphi(a_i\,;b_n\,;C).}

  We claim that the sequence has to stop after $\le m$ steps. 
  In fact, as (4) ensures $\langle b_i,C\rangle\notin{\EuScript D}$ for every $i$, from (3) we obtain ${\sim}\varphi(a_n\,;b_i\,;C_i)$ for every $i<n$.
  On the other hand, by (4) we have that $\varphi(a_i\,;b_n\,;C)$ for every $i<n$.
  Therefore, after reversing the order, the sequence $\langle a_i\,;b_i\,;C_i\ :\ i\le m\rangle$ contradicts the maximality of $m$.
\end{proof}
...
\begin{proof}
  Let $k$ be suffinciely large so that every $m$-coloring of a graph of size $k$ has a monocromatic subgraph of size $>m$.
  Let $\langle \bar a_i\,;b_i\,;C_i\ :\ i<k\rangle$ be a sequence witnessing instability as in (3) of Theorem~\ref{thm_epsilon_stable}.
  Then for every pair $i<n$ there is some $j<m$ such that ${\sim}\varphi(a_{j,n}\,;b_i\,;C_i)$.
  By the choice of $k$ there is a $j<m$ such that ${\sim}\varphi(a_{j,n}\,;b_i\,;C_i)$ obtains for $>m$ many $i$.
  Then we can extract a subsequence that contradicts the maximality of $m$.
\end{proof}
...
\begin{theorem}[ (conjecture)]\label{thm_stable_definable}
  ...
  Let ${\EuScript D}$ be approximable by $\varphi(x\,;z\,;X)$.
  Then there is $\pi(z\,;X)$, a small set of formulas of the form $\varphi(a\,;z\,;X)$, with $a\in{\EuScript U}^x$, such that

  \ceq{\hfill \langle b,C\rangle\in{\EuScript D}}{\Leftrightarrow}{\pi(b\,;C)}\hfill for every $b\in{\EuScript U}^z$ and every $C$.
\end{theorem}
...
% \end{theorem}
In this section we consider formulas of a particular form which is of some interest. 
