...
\begin{abstract}
  ...
  \noindent Recently, a classical approach to continuous structures has been proposed
  in \cite{clcl} and~\cite{Z} that properly extend the class of structures that
  falls under the scope of~\cite{BBHU} or~\cite{HI}. 
  These articles introduced the notion of structures with a standard sort.
  We discuss local stability in this context and to prove that every set externally definable by a stable formula is definable.
\end{abstract}
...
%   This is obtained by excluding equalities and inequalities from (i) and requiring that $C$ in (ii) is a compact set.
...
% We introduce a new sort ${\sf K}$ with the sole scope of conveniently describing classes of ${\EuScript F}$-formulas that only differ by the sets/predicates $C$ they contain.
% Let ${\EuScript F}_{\sf K}$ be defined as ${\EuScript F}$ but replacing (ii) with\smallskip
%
% \begin{itemize}
%   \item[iii.] $\tau(x\,;\xi)\in X$, where $X$ is a variable of sort ${\sf K}$.
% \end{itemize}
...
% Let $\varphi(X)\in{\EuScript F}_{\sf K}({\EuScript U})$ be given. Define
...
\begin{proof}
  ...
  1$\Rightarrow$2. 
  Negate (2).
  Pick $\varepsilon$ and $\langle a_i\,;b_i\,;C_i\,;\tilde C_i\ :\ i<\omega\rangle$ that are a counter example to (1) in Definition~\ref{defthm_epsilon_stable}.
  By the compactness of $K(S)$ in the Vietoris topology, for every $\delta$ there are $C$ and $\tilde C$ such that for infinitely many $i<\omega$ we have
  ...
  Pick $\delta$ such that $2\delta\subseteq\varepsilon$.
  Refine the sequence so that the inclusions above hold for every $i$.
  Then, as $C_i^{2\delta}\cap\tilde C_i=\varnothing$, for every $i<\omega$.
  Then $C^\delta\cap\tilde C_i=\varnothing$, for every $i<\omega$.
  And finally $C\cap\tilde C=\varnothing$.
\end{proof}
