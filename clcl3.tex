...
\begin{abstract}
  ...
  For each we prove that sets externally definable by stable formulas are definable in some appropiate sense.
\end{abstract}
...
\begin{definition}
  ...
  \item[ii.] $\tau\in C$, where $\tau$ is a term of sort ${\sf H}^n\times {\sf S}^m\to {\sf S}$, and $C$ a compact set.
  \end{itemize}
\end{definition}
...
\begin{fact}
  For every $C\subseteq C'$ there is $\tilde C$ disjoint from $C$ such that\smallskip
...
\begin{proof}
  ...
  The existential quantifier of sort {\sf H} is the only case that requires attention.
...
\begin{proof}
  ...
  Then $C\cap\tilde C$ for some $\tilde C\in{\EuScript D}=\varnothing$.
...
We say that $\varphi(x\,;z\,;C)$ is  \emph{finitarily stable\/} if for some $m<\omega$ there is no sequence $\langle a_i\,;b_i\ :\ i<m\rangle$ such that\smallskip

\ceq{1.\hfill \varphi(a_n\,;b_i\,;C)}{\wedge}{\neg\varphi(a_i\,;b_n\,;C)} for every $i<n<m$.
\end{definition}
...
\begin{proof}
  ...
  By Fact~\ref{fact_otto} there are $\tilde C_{n,i}$ such that 
...
\begin{remark}\label{rem_stable}
  ...
  \ceq{\hfill \sigma(a_n\,;b_i\,;C)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;\tilde C)}\hfill for every $i<n<\kappa$ or for every $n<i<\kappa$.\smallskip
\end{remark}
...
\begin{proof}
  ...
  The formula $\varphi(x\,;z\,;X)$ depends on $i$ and $n$ but, since $\kappa$ is a Ramsey cardinal, we can refine the sequence to obtain that the same $\varphi(x\,;z\,;X)$ for all $i<n<\kappa$.

  Once we replace ${\sim}\sigma(x\,;z\,;X)$ by ${\sim}\varphi(x\,;z\,;X)$ compactness applies and the existence of a sequence of length $\omega$ is equivalent to the existence of a sequence of length $\kappa$.
\end{proof}


Note also that, by the pigeonhole principle, we can strengthen the requirement in Definition~\ref{def_stable} by requiring that there is no $\langle a_i\,;b_i\,;C_i\,;\tilde C_i\ :\ i<\omega\rangle$ such that $\tilde C_i\cap C_i=\varnothing$ and\smallskip

  \ceq{\hfill \sigma(a_n\,;b_i\,;C_i)}{\wedge}{{\sim}\varphi(a_i\,;b_n\,;\tilde C_n)}\hfill for every $i<n<\kappa$ or for every $n<i<\kappa$.\smallskip
...
When $\sigma(x\,;z\,;X)$ is a formula and  ${\EuScript D}$ is involutive (2) in the above definition can be rephrased in a more convenint form.

\begin{remark}\label{rem_approx}
  If ${\EuScript D}$ is involutive the following are equivalent

  \ceq{2.\hfill\langle b,C\rangle\in {\EuScript D}}{\Leftarrow}{\varphi(a\,;b\,;C)}\hfill for every $b\in B$ and every $C$\phantom{.}\smallskip

  \ceq{2'\hfill\langle b,C\rangle\in{\sim}{\EuScript D}}{\Rightarrow}{{\sim}\varphi(a\,;b\,;C)}\hfill for every $b\in B$ and every $C$.
\end{remark}

\begin{proof}
  Let ${\EuScript C}=\{C\,:\,\varphi(a,b,C)\}$.
  This is also an an involutive set by Fact~\ref{fact_~definibile}.
...
Under the assunption of the remark it is easy to verify by compactness that in Definition~\ref{def_approx} the requirement \textit{for every small\/} $B$ can be replaced by \textit{for every finite\/} $B$.
...
\begin{proof}
  First we prove that any $p(x)$ defined as in (2) is maximal as soon as it is consistent~--~in fact, this only depends on ${\EuScript D}$ being involutive.
...
This contradicts stability (as noted after Remark~\ref{rem_stable}).
\end{proof}
...
\begin{proof}
  ...
  Therefore, $\langle a_i\,;b_i\,;C_i\,;\tilde C_i\ :\ i<\kappa\rangle$ contradicts stability (as noted after Remark~\ref{rem_stable}).
\end{proof}
