...
In this section  ${\EuScript D}$ is always a subset of ${\EuScript U}^z\times K(S)$, where $n$ has to be inferred from the context (tipically, $n=|X|$).
...
\begin{proof}
  ...
  For every $i<n$ we can pick some $j<\lambda$ such that ${\sim}\sigma(a_{i,j}\,;b_n\,;\tilde C)$.
  ...
  That is, there is some fixed $j<\lambda$ such that ${\sim}\sigma(a_{i,j}\,;b_n\,;\tilde C)$ holds for $i<n$ restricted to range over a subsequence of length $\kappa$.
